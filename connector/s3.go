...
func (conn *S3Connection) ListObjects(bucket string, prefix string) ([]S3Object, error) {
	...
	for {
		...
		if err != nil {
			log.Println("Failed to ListObjects for bucket %s, prefix %s: %s", bucket, prefix, err)
			return nil, err
		}
		for i := range result.Contents {
			s3Obj := S3Object{
				S3FullPath: *result.Contents[i].Key,
			}
...
func (conn *S3Connection) getCacheFilepath(key string) (string, error) {
	...
	if err != nil {
		log.Printf("Failed to make cacheFilepath %s absolute: %s",
			cacheFilepath, err)
		return "", err
	}
...
func (conn *S3Connection) CachedGet(bucket string, key string) (string, error) {
	...
	if err != nil {
		log.Printf("Failed to getCacheFilepath in CachedGet: %s", err)
		return "", err
	}
	...
	if err != nil {
		log.Println("Failed to cachedGet key: ", key)
...
func (conn *S3Connection) Get(bucket string, key string) (string, error) {
	cacheFilepath, err := conn.getCacheFilepath(key)
	if err != nil {
		log.Printf("Failed to getCacheFilepath in Get: %s", err)
		return cacheFilepath, err
	}
	err = os.MkdirAll(path.Dir(cacheFilepath), 0777)
	if err != nil {
		log.Printf("Couldn't create cache directory for cacheFilepath %s: %s",
			cacheFilepath, err)
		return cacheFilepath, err
	}
	w, err := os.Create(cacheFilepath)
	if err != nil {
		log.Printf("Couldn't create cache file for cacheFilepath %s: %s",
			cacheFilepath, err)
		return cacheFilepath, err
	}
	defer w.Close()
	...
	time.Sleep(100 * time.Millisecond)
	if err != nil {
		log.Printf("Failed to download key: %s", err)
